...
ALLOWED_SSID = ("223.185.36", "223.185.37")
...
##             checkin_10am = datetime.strptime('10:00:00', '%H:%M:%S').time()
##             checkin_1030am = datetime.strptime('10:30:00', '%H:%M:%S').time()
##             checkin_11am = datetime.strptime('11:00:00', '%H:%M:%S').time()
##             checkout_630pm = datetime.strptime('18:30:00', '%H:%M:%S').time()
#
##             day_status = 'Full Day'
##             expected_checkout = checkout_630pm
#
##             if checkin_time.time() <= checkin_1030am:
##                 expected_checkout = checkout_630pm
##             elif checkin_1030am < checkin_time.time() <= checkin_11am:
##                 checkin_dt = datetime.combine(now.date(), checkin_time.time())
##                 expected_checkout_dt = checkin_dt + timedelta(hours=8)
##                 expected_checkout = expected_checkout_dt.time()
##             else:  # After 11:00 AM
##                 expected_checkout = checkout_630pm
##                 day_status = 'Half Day'
#
##             attendance.append([name, date_str, time_str, '', 'Present', '', day_status])
##             update_sheet(attendance)
##             return True, expected_checkout.strftime('%H:%M:%S')
##         return False, None
#
##     elif action == 'checkout' and today_records:
##         last_record = today_records[-1]
##         checkin_time = datetime.strptime(last_record[2], '%H:%M:%S') if last_record[2] else None
##         checkout_time = datetime.strptime(last_record[3], '%H:%M:%S') if last_record[3] else None
#
##         if checkin_time and not checkout_time:
##             time_since_checkin = now - datetime.combine(now.date(), checkin_time.time())
##             checkin_10am = datetime.strptime('10:00:00', '%H:%M:%S').time()
##             checkin_1030am = datetime.strptime('10:30:00', '%H:%M:%S').time()
##             checkin_11am = datetime.strptime('11:00:00', '%H:%M:%S').time()
##             checkout_630pm = datetime.strptime('18:30:00', '%H:%M:%S').time()
#
##             expected_checkout = checkout_630pm
##             if checkin_1030am < checkin_time.time() <= checkin_11am:
##                 checkin_dt = datetime.combine(now.date(), checkin_time.time())
##                 expected_checkout_dt = checkin_dt + timedelta(hours=8)
##                 expected_checkout = expected_checkout_dt.time()
#
##             # Adjust expected checkout time as necessary
##             if time_since_checkin >= timedelta(hours=7) and now.time() >= expected_checkout:
##                 last_record[3] = time_str
##                 last_record[4] = 'Present'
##                 update_sheet(attendance)
##                 return True, None
##             return False, expected_checkout.strftime('%H:%M:%S')
##         return False, None
#
##     return False, None
#
...
def is_account_expired(username):
    expiration_time = timedelta(hours=7)  # Account expires after 19 hours
...
#         'hours': '',
#         'checkin_image': None  # New field for check-in image
#     } for name in names}
...
#             'allow_checkout': bool(checkin and not checkout),
#             'hours': hours,
#             'checkin_image': get_checkin_image_base64(name, today)  # Add check-in image
#         }
...
#     
# #     # ✅ Convert UTC to IST manually by adding 5 hours 30 minutes
# #     india_now = datetime.now() + timedelta(hours=5, minutes=30)
# #     date_str = india_now.strftime('%d/%m/%Y')
# 
# #     checkin = request.form.get(f'checkin_{name}', '').strip()
# #     checkout = request.form.get(f'checkout_{name}', '').strip()
# 
...
# #             checkout_630pm = datetime.strptime('18:30:00', '%H:%M:%S').time()
# 
# #             # Day status: Full Day or Half Day based on check-in time
# #             day_status = 'Full Day'
# #             if checkin_time.time() > checkin_11am:
# #                 day_status = 'Half Day'
# 
# #             last_record[2] = checkin + ':00'
# #             last_record[4] = 'Present'
# #             last_record[6] = day_status
# #             updated = True
#         
# #         # Checkout handling
# #         elif checkout and last_record[2] and not last_record[3]:
# #             checkin_time = datetime.strptime(last_record[2], '%H:%M:%S')
# #             checkin_datetime = datetime.combine(india_now.date(), checkin_time.time())
# #             time_since_checkin = india_now - checkin_datetime
# 
# #             checkin_10am = datetime.strptime('10:00:00', '%H:%M:%S').time()
# #             checkin_1030am = datetime.strptime('10:30:00', '%H:%M:%S').time()
# #             checkin_11am = datetime.strptime('11:00:00', '%H:%M:%S').time()
...
# #             expected_checkout = checkout_630pm
# #             if checkin_1030am < checkin_time.time() <= checkin_11am:
# #                 checkin_dt = datetime.combine(india_now.date(), checkin_time.time())
# #                 expected_checkout_dt = checkin_dt + timedelta(hours=8)
# #                 expected_checkout = expected_checkout_dt.time()
# 
# #             # Check if it's been 7 hours since check-in or if the user is admin
# #             if time_since_checkin >= timedelta(hours=7) and india_now.time() >= expected_checkout:
...
# #                 updated = True
# #             else:
# #                 flash(f"Checkout not allowed yet. Wait until {expected_checkout.strftime('%H:%M:%S')}.", "error")
# #                 return redirect('/admin_panel')
# #     else:
# #         # Handle new attendance entry
# #         checkin_time = datetime.strptime(checkin + ':00', '%H:%M:%S') if checkin else None
# #         day_status = 'Full Day'
# #         if checkin_time and checkin_time.time() > datetime.strptime('11:00:00', '%H:%M:%S').time():
# #             day_status = 'Half Day'
# #         attendance.append([name, date_str, checkin + ':00' if checkin else '', checkout + ':00' if checkout else '', 'Present' if checkin else 'Absent', '', day_status])
# 
# #     # Update the sheet if any changes were made or if it's a new attendance entry
# #     if updated or not today_records:
# #         try:
# #             update_sheet(attendance)
# #         except ConnectionResetError as e:
# #             print(f"ConnectionResetError in mark_attendance: {e}")
# #             flash("Network error: Could not update attendance. Please try again.", "error")
# #             return redirect('/admin_panel')
# 
# #     return redirect('/admin_panel')
...
#             if os.path.exists(person_dir):
#                 shutil.rmtree(person_dir)
#             # Delete any check-in images for this user
